...
const UpdateMyAccount = z
  ...
  .refine((input) => !input.includeAddress || input.city!.length > 0, {
    message: 'To pole jest wymagane',
    path: ['city'],
  })
  .refine((input) => !input.includeAddress || /^[0-9]{2}-[0-9]{3}$/.test(input.postalCode!), {
    message: 'Kod pocztowy musi odpowiadać formatowi 00-000',
    path: ['postalCode'],
  })
  .refine((input) => !input.includeAddress || /^\d+[A-Z]{0,1}$/.test(input.number!), {
    message: 'Wprowadź poprawny numer budynku',
    path: ['number'],
  })
  .refine((input) => !input.includeAddress || input.street!.length > 0, {
    message: 'To pole jest wymagane',
    path: ['street'],
  })
  .superRefine((input) => {
