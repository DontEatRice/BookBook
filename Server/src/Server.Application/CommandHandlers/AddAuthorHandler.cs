...
public sealed record AddAuthorCommand(Guid Id, string FirstName, string LastName, int BirthYear, string? PhotoLink) : IRequest;
...
public sealed class AddAuthorHandler : IRequestHandler<AddAuthorCommand>
{
    ...
    {
        var author = Author.Create(request.Id, request.FirstName, request.LastName, request.BirthYear, request.PhotoLink);
